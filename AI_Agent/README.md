...
### 3. ⚡ 비동기 병렬 터보 모드 (`!` 트리거)
* 메시지 맨 앞에 **`!`**를 붙여 전송(예: `!폴더 3개 파일 개수 세어줘`)하면, AI가 여러 개의 도구를 호출할 때 순차 대기 없이 **`Promise.all`로 동시에 병렬 실행**하여 처리 속도를 3~5배 획기적으로 단축합니다.
* 평상시에는 안전한 순차 동기 모드로 동작하여 데이터 꼬임을 원천 차단하고, 사용자가 명시적으로 원할 때만 터보 모드를 가동합니다.

### 4. ⌨️ 스마트 단축어 & 인텔리센스 자동완성
* **`!` 트리거**: 비동기 병렬 터보 모드 실행 안내 및 가동.
* **`#` 트리거**: 자주 사용하는 단축 프롬프트(인사, 프롬프트 개선 등) 자동완성 팝업.
* **`@` 트리거**: `@오늘`, `@현재시간`, `@날짜` 등 실시간 시스템 일시를 텍스트창에 즉시 삽입.
* **`?` 트리거**: 단축어 및 시스템 도움말 팝업 호출.

### 5. 🎨 모던 UI/UX
* **도구 호출 접기/펼치기**: 도구 실행 과정(인자, 실행 결과, ⚡터보 뱃지)이 깔끔한 아코디언 UI로 렌더링되어 대화창이 정돈됩니다.
