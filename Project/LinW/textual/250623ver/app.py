...
threading.Thread(target=frame_update, daemon=True).start()
...
class Slot(Static):
    ...
    def on_click(self):
        ...
        label = f"✅ {self.renderable}" if self.run else f"🟨 {self.renderable}"
...
class MyApp(App):
    ...
    def on_mount(self):
        self.set_interval(0.2, self.loop)


    def loop(self):
        if not pause:
            for idx, name in enumerate(hp.keys()):
                if self.query_one(f"#hp_{idx}").run:
                    key = hp[name]["key"]
                    ...
                    self.query_one(f"#hp_{idx}").children[0].update(f"✅ {name} ({hp_calc})")
                else:
                    self.query_one(f"#hp_{idx}").children[0].update(f"🟨 {name}")

            for idx, name in enumerate(img.keys()):
                if self.query_one(f"#img_{idx}").run:
                    self.query_one(f"#img_{idx}").children[0].update(f"✅ {name} ({self.idx})")
                else:
                    self.query_one(f"#img_{idx}").children[0].update(f"🟨 {name}")

            for idx, name in enumerate(timer.keys()):
                if self.query_one(f"#timer_{idx}").run and not cooling[name]:
                    key = timer[name]["key"]
