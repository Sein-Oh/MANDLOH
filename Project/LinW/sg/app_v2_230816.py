...
def slot_cool_run(idx, sec):
    print(f"slot cool run-{idx}, {sec}")
    global slot_cool_ary
    slot_cool_ary[idx] = True
    threading.Timer(sec, slot_cool_down, [idx]).start()
    return
...
def update_params(param):
    global slot_img_path_ary
    for key in param.keys():
        if "path" in key:
            idx = int(key.split("_")[0][-1]) - 1
            slot_img_path_ary[idx] = param[key]
        else:
...
hp1_lbl = sg.Column([
    ...
    [sg.Text("결과값"), sg.Input("0", key="hp1_inp_res", expand_x=True, justification="center", readonly=True)],
])
...
hp2_lbl = sg.Column([
    ...
    [sg.Text("결과값"), sg.Input("0", key="hp2_inp_res", expand_x=True, justification="center", readonly=True)],
])
...
hp3_lbl = sg.Column([
    ...
    [sg.Text("결과값"), sg.Input("0", key="hp3_inp_res", expand_x=True, justification="center", readonly=True)],
])
...
slot1_lbl = sg.Column([
    ...
    [sg.Text("결과값"), sg.Input("0.0", key="slot1_inp_res", expand_x=True, justification="center", size=(16,None), readonly=True)],
])
...
slot2_lbl = sg.Column([
    ...
    [sg.Text("결과값"), sg.Input("0.0", key="slot2_inp_res", expand_x=True, justification="center", size=(16,None), readonly=True)],
])
...
slot3_lbl = sg.Column([
    ...
    [sg.Text("결과값"), sg.Input("0.0", key="slot3_inp_res", expand_x=True, justification="center", size=(16,None), readonly=True)],
])


layout = [
    [sg.Menu([["  메뉴  ", ["설정 저장하기", "설정 불러오기", "종료하기"]], [" 도움말 ", ["사용방법", "문의하기"]]])],
    [sg.Frame("정보", [
    ...
    [sg.Frame("아두이노", connection, key="frame_arduino")],
    [sg.Frame("타이머", timer)],
...
default_param = {
    ...
    "hp1_inp_roi" : "0,0,200,20",
    "hp1_inp_thres" : "210",
    "hp1_inp_key" : "1",
    "hp1_inp_cool" : "3",

    "hp2_inp_roi" : "0,0,200,20",
    "hp2_inp_thres" : "210",
    "hp2_inp_key" : "2",
    "hp2_inp_cool" : "3",

    "hp3_inp_roi" : "0,0,200,20",
    "hp3_inp_thres" : "210",
    "hp3_inp_key" : "3",
    "hp3_inp_cool" : "3",
...
param_keys = list(default_param.keys())
    ...
    if event == sg.WINDOW_CLOSED or event == "종료하기":
        ...
        for key in param_keys:
            if key != None:
                if "path" in key:
                    idx = int(key.split("_")[0][-1]) - 1
                    param[key] = slot_img_path_ary[idx]
                else:
                    param[key] = window[key].get()
        with open("userdata.json", "w") as file:
            json.dump(param, file, indent=4)

    elif event == "설정 불러오기":
        file_path = sg.popup_get_file("불러올 파일을 선택하세요.", title="불러오기", file_types=(('Json', '*.json'),))
        param = load_data(file_path)
        update_params(param)
