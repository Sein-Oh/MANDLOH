...
def send_keys(keys, frame):
    ...
    for key in key_ary:
        if "-" in key:
            ...
            mx, my = list(map(float, key.split(",")))
            if app_data["capture"] == "fullscreen":
                mx, my = int(mx * capture_size[1]), int(my * capture_size[0])
            else:
                x1, y1, x2, y2 = get_win_size(target_hwnd)
                mx, my = int(x1 + (mx * (x2-x1))), int(y1 + (my * (y2-y1)))
...
class MainWindow(Tk):
    def __init__(self):
        ...
        self.info = Label(self, text="Mouse:")
        self.info.grid(row=1, column=0, columnspan=2)

        for idx, k in enumerate(slot.keys()):
            Checkbutton(self, text=k, command=lambda text=k: self.toggle_run(text)).grid(row=idx+2, column=0, sticky="w")
            Label(self, text="", name=k).grid(row=idx+2, column=1, sticky="w")

        self.cam = dxcam.create(output_color="BGR")
        self.cam.start()
        capture_size = self.cam.get_latest_frame().shape
        self.update()
    ...
    def update(self):
        if app_data["capture"] == "fullscreen":
            ...
            cx1, cy1, cx2, cy2 = get_win_size(target_hwnd)
            frame = self.cam.get_latest_frame()[cy1:cy2, cx1:cx2]
        ...
        if app_data["capture"] == "fullscreen":
            mx, my = round(x / capture_size[1], 3), round(y / capture_size[0], 3)
        else:
            mx, my = round((x-cx1)/(cx2-cx1), 3), round((y-cy1)/(cy2-cy1), 3)
        self.info.configure(text=f"Mouse: {mx},{my}")
