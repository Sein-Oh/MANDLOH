"""wiki-mcp-server.py - LLM-Native 지식 베이스(Wiki) FastMCP 서버.

주요 특징:
1. 마크다운(`.md`) 기반 로컬 영구 지식 베이스 (사람과 AI 모두 열람/편집 가능)
2. `[[문서명]]` 양방향 위키 링크 자동 추출 및 역링크(Backlinks) 인덱싱
3. 14 CFR Part 33 등 복잡한 규정/엔지니어링 문서의 다단계 상호 참조(Cross-reference) 완벽 지원
4. Windows 네이티브 휴지통 안전 삭제 (Safe Delete)
5. CORS 전체 허용 및 HTTP / SSE / stdio 전송 방식 지원 (기본 포트: 8003)

제공 도구 (총 6개):
1. wiki_read - 위키 문서 본문 및 링크/역링크(Backlinks) 통합 조회
2. wiki_create_or_update - 위키 문서 생성 또는 내용 갱신(덮어쓰기/덧붙이기)
3. wiki_search - 키워드, 태그, 카테고리 기반 위키 지식 검색
4. wiki_list_all - 전체 위키 문서 목차 및 카테고리별 트리 조회
5. wiki_backlinks - 특정 문서를 참조하고 있는 모든 연관 문서(역링크) 추적
...
# 위키 마크다운 파일 저장소 기본 디렉토리
WIKI_DIR = Path(__file__).parent / "wiki_data"


# ==============================================================================
# 1. 헬퍼 유틸리티 (YAML 파서, 위키 링크 파서, 휴지통)
# ==============================================================================

def ensure_wiki_dir() -> Path:
    """위키 데이터 저장 디렉토리를 생성하고 반환합니다."""
    WIKI_DIR.mkdir(parents=True, exist_ok=True)
    return WIKI_DIR


def sanitize_filename(name: str) -> str:
    """파일명으로 사용할 수 없는 특수문자를 언더스코어로 안전하게 치환합니다."""
    clean = re.sub(r'[\\/:*?"<>|\r\n\t]', '_', name).strip()
...
def get_all_wiki_files() -> List[Path]:
    """저장소 내의 모든 마크다운 위키 파일 목록을 반환합니다."""
    ensure_wiki_dir()
    return sorted(WIKI_DIR.glob("*.md"))


def build_backlinks_index() -> Dict[str, Set[str]]:
    """전체 위키 문서를 스캔하여 대상 문서별 역링크(이 문서를 참조하는 문서들) 색인을 생성합니다."""
    ...
    for f in files:
        ...
        for target in outgoing_links:
            target_clean = sanitize_filename(target).lower()
            if target_clean not in backlinks:
                backlinks[target_clean] = set()
            backlinks[target_clean].add(meta.get("title") or source_title)
...
# ==============================================================================
# 2. 초기 샘플 데이터 시딩 (14 CFR Part 33 항공 엔진 감항 기준)
# ==============================================================================

def init_sample_cfr_data():
    """14 CFR Part 33 상호 연계 감항 기준 샘플 위키 문서를 기본 생성합니다."""
    ensure_wiki_dir()
    sample_files = list(WIKI_DIR.glob("*.md"))
    if sample_files:
        return
...
"""
    f1 = WIKI_DIR / "33.28_Engine_Control_Systems.md"
    f1.write_text(build_markdown_document("14 CFR § 33.28 - Engine Control Systems", doc_33_28, ["CFR", "Part33", "FADEC", "제어시스템"], "항공감항규정"), encoding="utf-8")
...
"""
    f2 = WIKI_DIR / "33.75_Safety_Analysis.md"
    f2.write_text(build_markdown_document("14 CFR § 33.75 - Safety Analysis", doc_33_75, ["CFR", "Part33", "Safety", "안전성평가"], "항공감항규정"), encoding="utf-8")
...
"""
    f3 = WIKI_DIR / "33.87_Endurance_Test.md"
    f3.write_text(build_markdown_document("14 CFR § 33.87 - Endurance Test", doc_33_87, ["CFR", "Part33", "Endurance", "내구성시험"], "항공감항규정"), encoding="utf-8")
...
async def wiki_read(
    title: str = Field(
        description="읽을 위키 문서의 제목 또는 파일명 (예: '33.28_Engine_Control_Systems', 'FastMCP_설정')",
        examples=["33.28_Engine_Control_Systems", "33.75_Safety_Analysis"],
    ),
) -> str:
    """[위키 문서 읽기] 위키 문서의 본문 내용과 함께, 본문이 참조하는 링크 및 이 문서를 참조하는 역링크(Backlinks) 목록을 통합 반환합니다."""
    def _run():
        ensure_wiki_dir()
        ...
        clean_name = sanitize_filename(t_str)
        target_file = WIKI_DIR / f"{clean_name}.md"
        if not target_file.exists():
            candidates = [f for f in WIKI_DIR.glob("*.md") if clean_name.lower() in f.stem.lower()]
            if candidates:
...
async def wiki_create_or_update(
    title: str = Field(
        ...
        examples=["33.67_Fuel_System", "NeuJS_아키텍처"],
    ),
    content: str = Field(
    ...
    category: str = Field(
        default="일반",
        description="문서가 속한 대분류 카테고리 (예: '항공감항규정', '프로젝트', '개인메모')",
    ),
    ...
    """[위키 문서 생성/갱신] 새로운 위키 문서를 작성하거나 기존 문서 내용을 보강 및 갱신합니다."""
    def _run():
        ensure_wiki_dir()
        t_str = str(title).strip() if title and isinstance(title, str) else ""
        if not t_str:
        ...
        doc_text = build_markdown_document(t_str, final_content, final_tags, category_val, created_at)
...
async def wiki_search(
    ...
    category: Optional[str] = Field(
        default=None,
        description="특정 카테고리로 필터링할 경우 지정 (예: '항공감항규정')",
    ),
) -> str:
    """[위키 지식 검색] 키워드, 태그, 카테고리를 기준으로 관련된 위키 문서들을 신속하게 검색합니다."""
    def _run():
        files = get_all_wiki_files()
        if not files:
        ...
        for idx, r in enumerate(results, 1):
            ...
            lines.append(f"{idx}. 📄 **`{r['name']}`** - {r['title']}{reasons_str}")
            lines.append(f"   - 분류: `{r['category']}`{tags_str}")
...
async def wiki_list_all(
    category: Optional[str] = Field(
        default=None,
        description="특정 카테고리의 문서만 조회하려면 카테고리명을 지정하세요",
    ),
) -> str:
    """[위키 전체 목차 조회] 지식 베이스에 축적된 모든 문서 목록을 카테고리별 트리 형태로 확인합니다."""
    def _run():
        files = get_all_wiki_files()
        if not files:
        ...
        lines = [
            f"📚 **[LLM Wiki 전체 목차 색인 (총 {len(files)}개 문서)]**",
            "",
        ]
...
async def wiki_backlinks(
    ...
    """[역링크 추적] 특정 위키 문서를 참조([[링크]])하고 있는 모든 상위 조항 및 연관 문서를 역추적합니다."""
    def _run():
        t_str = str(title).strip() if title and isinstance(title, str) else ""
        if not t_str:
            return "❌ 역링크를 추적할 대상 문서명을 지정해주세요."

        clean_target = sanitize_filename(t_str).lower()
        backlinks_idx = build_backlinks_index()
...
async def wiki_delete_to_trash(
    ...
    def _run():
        ...
        if not target_file.exists():
            return f"❌ 삭제할 위키 문서 `{title}`이(가) 존재하지 않습니다."

        try:
            send_to_recycle_bin(target_file)
            return f"🗑️ 위키 문서 `{clean_name}.md`을(를) OS 휴지통으로 안전하게 이동했습니다 (Safe Delete)."
